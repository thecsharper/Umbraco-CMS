...
public class LoggerAllocationBenchmark
{
    ...
    public void Baseline()
    {
        ...
        {
            OriginalDebugSignature(GetType(), "DeleteFromIndex with query: {Query} (found {TotalItems} results)", _rawQuery, _totalItemCount);
        }
    }
    ...
    public void NewOverload2()
    {
        for (var i = 0; i < 1000; i++)
        {
            NewDebugSignature(GetType(), "DeleteFromIndex with query: {Query} (found {TotalItems} results)", _rawQuery, _totalItemCount);
        }
    }
